...
- **コンセプト（2026-07-24 再定義）**: ターゲットは「生き急いでいる人・意識の高い人」。表示の主軸は経過率ではなく**残量**（「残り 44.5786%」「残り163日 04:12:08」）。小数点以下まで常に動き続ける数字で、時間が失われていく焦りを演出する。
...
- 外部パッケージ依存: **ゼロ**（2026-07-24 に Firebase / GoogleSignIn を撤去。Apple 純正フレームワークのみ）
...
| タブ | View | 役割 |
|---|---|---|
| Home | `HomeView` | 残り時間の計器盤（年の残り%・残量バー・365日グリッド・使途記録） |
| Notes | `NotesView` | 使途記録の閲覧・投稿・削除（SwiftData ローカル） |
| Settings | `SettingsView` | 表示設定（週の始まり・日付/時刻形式）とデータ保存先の説明 |

### 各画面の意図
- **WelcomeView**: `welcomeImage` を約1.5秒表示してフェードアウトするスプラッシュ。`showWelcomeScreen` バインディングで `ContentView` に遷移。
- **HomeView（2026-07-24 全面刷新）**: ダーク計器盤。構成は上から (1) 時計（秒はアンバー）、(2) ヒーロー＝年の残り%（小数6桁、`TimelineView(.animation)` で約20fps駆動）＋「残り◯日 hh:mm:ss」カウントダウン＋残量バー、(3) DAY/WEEK/MONTH の残量バー（小数4桁）、(4) 365日グリッド `YEAR IN DAYS`（記録した日=アンバー、過ぎた日=暗色、今日=白、未来=輪郭のみ。記録日数と分母も表示）、(5) 使途記録（最新3件。各記録に「= 年の0.27%」の重みスタンプ）。常時駆動が必要な区画だけを `TimelineView` 配下に置き、グリッド・記録一覧は再描画しない。`ContentView` でアプリ全体を `preferredColorScheme(.dark)` に固定。
- **NotesView（2026-07-24 刷新）**: 使途記録の一覧・投稿・削除。SwiftData（`@Query` + `modelContext`）で完結し、ログイン不要。30文字上限・1日1件・文字数カウンタは維持。入力プロンプトは「今日は年の0.27%。何に使った？」、各記録に「= 年の◯%」の重みスタンプ。DS トークンでダーク盤面化。投稿/削除時に App Group へ最新記録を書き `WidgetCenter.reloadAllTimelines()`。
- **SettingsView（2026-07-24 刷新）**: 認証 UI を削除。「表示」セクション（週の始まり・日付/時刻形式、`AppSettings`）と「データ」セクション（ローカル保存の説明）のみ。
...
**なし（2026-07-24 廃止）。** ログイン不要で全機能が使える。旧 Google サインイン＋Firebase Auth は依存ごと削除した（`AuthenticationManager` / `LoginView` / `UserSettingsView` / AppDelegate / GoogleService-Info 参照 / Info.plist の Google URL スキーム）。

---

## 5. ウィジェット

- 別ターゲット `DayTracerWidgets/`。小・中・大の3サイズ。2026-07-24 に「残量の計器盤」へ全面刷新（ダーク盤面固定）。
...
- **期間ヘルパー（2026-07-24 追加）**: `dayInterval` / `weekInterval` / `monthInterval` / `yearInterval` — ウィジェットの `timerInterval` 系 API に渡す `DateInterval`。進捗計算と同じ境界を返す。
...
## 6.5 デザインシステム（`DesignTokens.swift` / 2026-07-24 導入）
...
- **外部パッケージ依存ゼロ（2026-07-24〜）**: Firebase / GoogleSignIn の SPM 依存を撤去済み。Apple 純正フレームワークのみ（SwiftUI / SwiftData / WidgetKit）。旧「Xcode 15.0 世代へのピン留め」制約は依存ごと消滅。gRPC の CFBundleIdentifier ビルドエラーも依存撤去により根治。
- `GoogleService-Info.plist` はプロジェクト参照を削除済み（ファイル自体はリポジトリ外 `../../../Programs/` にあったもの。もう不要）。
