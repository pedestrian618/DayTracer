...
### 次の予定
- **Phase 2**: App Group 定数・ノート保存の共通化。`SharedConfig`（suite 名・キー名）と `SharedNoteStore`（read/write）を新規作成し、`project.pbxproj` を編集して両ターゲットに登録。`NotesView`（書く側）と `Provider`（読む側）を差し替え。
